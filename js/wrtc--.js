...
function mozVidFreezeModSdp(sdp){
  ...
  var wr_set_local_mute = function(obj, callId, mediaFlag, cb) {
    var senders = obj.peerConnection.getSenders();
    senders.forEach(function(element) {
      if (element.track.kind == mediaFlag)
        element.track.enabled = false;
    });
    cb(100, "Ok");
  };

  var wr_set_local_unmute = function(obj, callId, mediaFlag, cb) {
    var senders = obj.peerConnection.getSenders();
    senders.forEach(function(element) {
      if (element.track.kind == mediaFlag)
